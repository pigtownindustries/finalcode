...
// Icon Imports (dari Lucide React)
import {
  DollarSign,
  Users,
  Settings,
...
export function OwnerDashboard(): JSX.Element {
  ...
  const tabsConfig = [
    { value: "overview", label: "Overview", icon: Globe, shortLabel: "Home" },
  ...
  useEffect(() => {
    const initializeDashboard = async () => {
      try {
        ...
        toast({
          title: "Peringatan",
          description: "Beberapa data mungkin tidak tersedia. Silakan refresh halaman.",
          variant: "default",
        });
      } finally {
        setLoading(false);
  ...
  const fetchUserData = async () => {
    try {
      console.log("🔐 Mendapatkan user yang login...");
      ...
      console.log("✅ User login ditemukan:", currentUser.id);

      // Langsung set email dari auth (yang pasti ada)
      setAccountSettings((prev) => ({
        ...prev,
        email: currentUser.email || "",
        pin: "", // PIN akan diisi nanti jika ada
        currentPassword: "",
      }));

      // Coba dapatkan profile (jika ada)
      console.log("🔍 Mencari profile user...");
      const userProfile = await getUserProfile(currentUser.id);

      if (userProfile && userProfile.pin) {
        console.log("✅ PIN ditemukan di profile");
        setAccountSettings(prev => ({
          ...prev,
          pin: userProfile.pin
        }));
      } else {
        console.log("ℹ️ PIN tidak ditemukan, menggunakan default");
      }

    } catch (error) {
      console.log("⚠️ Error mengambil data user, tapi tidak apa-apa");
    }
  }
  ...
  const handleSettingsSubmit = async (e: React.FormEvent) => {
    ...
    if (accountSettings.pin.length !== 6) {
    ...
    try {
      ...
      if (currentUser) {
        if (currentUser.email !== accountSettings.email) {
          await supabase.auth.updateUser({ email: accountSettings.email })
        }
        if (accountSettings.newPassword) {
          await supabase.auth.updateUser({ password: accountSettings.newPassword })
        }
        await supabase.from("users").update({ email: accountSettings.email, pin: accountSettings.pin }).eq("id", currentUser.id)
      }
              ...
              <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
                ...
                <DialogContent className="sm:max-w-md backdrop-blur-xl bg-white/95 dark:bg-slate-900/95 border border-white/20 dark:border-slate-700/50 shadow-2xl">
                  ...
                  <form onSubmit={handleSettingsSubmit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="pin" className="text-sm font-medium">PIN Keamanan (6 digit)</Label>
                      <div className="relative">
                        <Input
                          id="pin"
